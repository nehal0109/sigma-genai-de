...
def sql_checker_node(state: CheckerState) -> dict:
    ...
    ~4 lines of code.
    """
    has_where = " where " in f" {state['sql'].lower()} "
    reason = "WHERE clause present; query is scoped." if has_where else "No WHERE clause found; full-table scans are blocked."
    return {"is_safe": has_where, "check_reason": reason}
...
def safe_executor_node(state: CheckerState) -> dict:
    ...
    try:
        ...
        result = df.to_string(index=False, max_rows=10) if not df.empty else "Query returned 0 rows."
...
def build_checker_graph():
    ...
    g.add_node("blocked", safe_executor_node)   # unsafe path
